...
import com.gabbas.quizapp_gabbas.databinding.ActivityRegisterBinding
...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun capturePhoto() {
        ...
        val photoFile = File.createTempFile("register_face_", ".jpg", cacheDir)
    ...
    private fun registerFace() {
        ...
        val requestUrl = Constants.BACKEND_URL + "/register"
        ...
        setLoading(true)
        Log.d("FaceAuth", "Backend URL = " + Constants.BACKEND_URL)
        Log.d("FaceAuth", "Register URL = $requestUrl")
        showToast(requestUrl)

        uiScope.launch {
            try {
                withContext(Dispatchers.IO) {
                    sendRegisterRequest(email, photoFile, requestUrl)
                }
                showToast("Inscrit avec succès")
                finish()
            } catch (exception: Exception) {
                Log.e("FaceAuth", "Register exception", exception)
                showToast(exception.message ?: "Erreur lors de l'inscription")
            } finally {
                setLoading(false)
            }
        }
    ...
    private fun sendRegisterRequest(email: String, photoFile: File, requestUrl: String) {
        ...
        okHttpClient.newCall(request).execute().use { response ->
            ...
            Log.d("FaceAuth", "Register response body = $responseBody")
            if (!response.isSuccessful) {
                throw IOException(responseBody.ifBlank { "Erreur lors de l'inscription" })
            }
        }
    }
    ...
    private fun setLoading(isLoading: Boolean) {
        ...
        binding.btnRegister.isEnabled = !isLoading && capturedPhotoFile != null
    }
